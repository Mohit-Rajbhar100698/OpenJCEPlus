...
public class TestArguments {
    ...
    public static Stream<Arguments> aesgcmKeySizesAndJCEPlusProviders() {
        int[] aesgcmKeySizes = {128, 192, 256};
        return keySizesAndJCEPlusProviders(aesgcmKeySizes);
    }

    /**
     * Provides active OpenJCEPlus* providers
     *
     * @return A stream of active TestProvider.
     */
    public static Stream<TestProvider> activeJCEPlusProviders() {
        List<TestProvider> activeJCEPlusProviders = getEnabledProviders();

        if (activeJCEPlusProviders.isEmpty()) {
            throw new IllegalArgumentException("No test providers found, unlikely this is what was asked for.");
        }
        return activeJCEPlusProviders.stream();
    }    
