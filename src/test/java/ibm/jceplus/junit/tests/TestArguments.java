...
public class TestArguments {
    ...
    /**
     * Provides enabled OpenJCEPlus* providers for SHA3_384 tests
     *
     * @return A stream of enabled TestProvider.
     */
    public static Stream<TestProvider> sha3_384JCEPlusProviders() {
        List<TestProvider> sha3_384ActiveProviders = getEnabledProviders();

        if (sha3_384ActiveProviders.isEmpty()) {
            throw new IllegalArgumentException("No test providers found, unlikely this is what was asked for.");
        }
        return sha3_384ActiveProviders.stream();
    }
